...
## Overview
The Usage Dashboard brings structured analytics to FlowPrompter so users can understand how dictation impacts their daily workflow. It aggregates per-session data into daily metrics, estimates time saved vs. typing, surfaces top applications, and tracks usage streaks. The dashboard ships as the first tab inside Settings and is powered by the `AnalyticsManager` for persistence and calculations.

## Key Components
| Component | Location | Responsibility |
|-----------|----------|----------------|
| `UsageMetrics` | `FlowPrompter/Domain/Models/UsageMetrics.swift` | Codable data model storing daily aggregates, lifetime totals, per-app stats, and streak data |
| `AnalyticsManager` | `FlowPrompter/Application/Managers/AnalyticsManager.swift` | Records sessions, persists metrics (`usage_metrics.json`), exposes queries for the dashboard |
| `UsageDashboardView` | `FlowPrompter/Presentation/Dashboard/UsageDashboardView.swift` | Main SwiftUI view combining period picker, metric cards, chart, app breakdown, streak section |
| `MetricCardView` | `FlowPrompter/Presentation/Dashboard/MetricCardView.swift` | Reusable card component with icon, value, subtitle, and trend badge |
| `UsageChartView` | `FlowPrompter/Presentation/Dashboard/UsageChartView.swift` | Swift Charts bar visualization with metric toggle (words / sessions / time saved) |
| `AppBreakdownView` | `FlowPrompter/Presentation/Dashboard/AppBreakdownView.swift` | Lists top apps plus relative share of sessions |
...
3. `UsageMetrics` updates lifetime totals, daily aggregates, per-app breakdown, and streak counters. Data persists to `~/Library/Application Support/FlowPrompter/usage_metrics.json`.
...
- `SettingsWindowController` and the `Settings {}` scene in `FlowPrompterApp` inject the shared manager from `AppDependencies` so both the Settings window and previews work.
